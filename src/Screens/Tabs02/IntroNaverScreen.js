...
import {StatusBar,SafeAreaView,PermissionsAndroid, Platform, TouchableOpacity, View,StyleSheet,Image,Dimensions,PixelRatio,Text,ToastAndroid,ActivityIndicator,Animated,Alert,ImageBackground,BackHandler} from 'react-native';
...
const baseEupmyeonZoomMaker = Platform.OS === 'ios' ? 13 : 13;
...
class IntroScreen extends React.Component {
    ...
    onRegionChangeComplete3 = async(region) => {  
        ...
        if ( baseZoom > baseEupmyeonZoomMaker ) { // 매물 view Articles           
            ...
            if ( this.state.markerNowFocus !== 'article') {
                ...
                if ( this.state.apiLatitude === null && showMaemul ) {
                    await this.setState({
                        apiLatitude : centerLat,
                        apiLongitude: centerLon
                    })
                    ...
                    const twoPointDistance = await CommonFunction.getDistanceBetweenPoints( this.state.apiLatitude,this.state.apiLongitude,centerLat,centerLon);
                    if ( twoPointDistance > 8000 && showMaemul) {                        
                        copyyfilterData = await this.API_getArticles(centerLat,centerLon);
                        await this.setState({
                            markerNowFocus : 'article',
            ...
            */
            ...
            if ( markerNowFocus !== 'eupmyeon' && showMaemul && showAgent) {
                if ( this.state.apiLatitude2 === null ) {
                    await this.setState({
                        apiLatitude2 : centerLat2,
                        apiLongitude2: centerLon2,
                        moreLoading:false
                    })
                    ...
                    if ( showMaemul ) this.setUpdatePermision();
                }else{
                    const twoPointDistance = await CommonFunction.getDistanceBetweenPoints( this.state.apiLatitude2,this.state.apiLongitude2,centerLat2,centerLon2);                        
                    if ( twoPointDistance > 30000 && showMaemul ) {     
                        copyyfilterData = await this.API_getUrban(centerLat2,centerLon2);
                        await this.setState({
                            markerNowFocus : 'eupmyeon',
                            apiLatitude2 : centerLat2,
            ...
            }else if ( markerNowFocus !== 'eupmyeon' && !showMaemul && showAgent) {                
                this.setState({markerNowFocus : 'eupmyeon'}) 
                this.setUpdatePermision();
            }else{
                const twoPointDistance = await CommonFunction.getDistanceBetweenPoints( this.state.apiLatitude2,this.state.apiLongitude2,centerLat2,centerLon2);
                if ( twoPointDistance > 5000 && showMaemul) {
                    copyyfilterData = await this.API_getUrban(centerLat2,centerLon2);
                    await this.setState({
                        markerNowFocus : 'eupmyeon',
                        apiLatitude2 : centerLat2,
    ...
    renderMapMarkers = () => {
        ...
        if ( this.props.mainMapViewTarget.maemul ) { //매물하고중개인            
            if ( this.state.markerNowFocus == 'article' ) {
                ...
                return (
                    <>
                    {
                        markerDataArticle.length > 0 &&
                        markerDataArticle.map((marker,index) => (
                            ( !CommonUtil.isEmpty(marker.fast_deal) && !CommonUtil.isEmpty(this.props.userToken) ) && marker.fast_deal.fast_deal_pk !== '0' ?
                            <Marker 
                                key={index}
                                coordinate={{latitude: marker.apart_lat, longitude: marker.apart_lon}}
                                image={articleNaverMarker}
                                height={CommonUtil.dpToSize(60*1.5)}
                                width={CommonUtil.dpToSize(86*1.4)}
                                caption={{
                                    ...strArticleCaption,
                                    text:
                                        marker.rent_avg_price > 0 ? 
                                        '급매 ' + CommonFunction.convertMillionComma(marker.fast_deal.trade_fast_deal) +'억    \n\n' + 
                                        marker.trade_avg_title+'억   '+ marker.rent_avg_title+'억   ' +'\n\n'
                                        : '급매 ' + CommonFunction.convertMillionComma(marker.fast_deal.trade_fast_deal) +'억    \n\n'+marker.trade_avg_title+'억        '+ '  -      ' +'\n\n'
                                }}
                            ...
                            <Marker 
                                key={index}
                                coordinate={{latitude: marker.apart_lat, longitude: marker.apart_lon}}
                                image={articleDefaultMarker}
                                height={CommonUtil.dpToSize(47*1.4)}
                                width={CommonUtil.dpToSize(86*1.4)}
                                caption={{
                                    ...strArticleCaption,
                                    text:
                                    marker.rent_avg_price > 0 ? marker.trade_avg_title+'억   '+ marker.rent_avg_title+'억 ' +'\n\n'
                                        : marker.trade_avg_title+'억          '+ '  -                 ' +'\n\n'
                                }}
                                onClick={() => this.moveDetail(marker)}
                            />            
                ...
                return (
                    markerDataEupmyeon.length > 0 &&
                    markerDataEupmyeon.map((marker,index) => (   
                        marker.price_rate2 == 100 ?
                        <Marker 
                            key={index}                             
                            coordinate={{latitude: marker.apart_lat, longitude: marker.apart_lon}}
                            image={areaDefaultMarker}
                        ...
                        <Marker 
                            key={index}                             
                            coordinate={{latitude: marker.apart_lat, longitude: marker.apart_lon}}
                            image={areaPlusMarker}
                            height={CommonUtil.dpToSize(95)}
                            width={CommonUtil.dpToSize(100)}                            
                            caption={{
                                ...strCaption,                            
                                text:'+'+((marker.price_rate2-100).toFixed(2))+'\n\n'+marker.name+"\n"+marker.trade_avg_title+'억\n\n'
                            }}
                        />
                        :
                        <Marker 
            ...
            }else if ( this.state.markerNowFocus == 'sigungu' ) {                
                const strCaption = Platform.OS === 'ios' ? iosCaption : androidCaption;
                return (
                    markerDataSigungu.length > 0 &&
                    markerDataSigungu.map((marker,index) => (
                        marker.price_rate2 == 100 ?
                        <Marker 
                            key={index}                             
                            coordinate={{latitude: marker.apart_lat, longitude: marker.apart_lon}}
                ...
                const strCaption = Platform.OS === 'ios' ? iosCaption : androidCaption;
                return (
                    markerDataSido.length > 0 &&
                    markerDataSido.map((marker,index) => (
                        marker.price_rate2 == 100 ?
                        <Marker 
                            key={index}                             
                            coordinate={{latitude: marker.apart_lat, longitude: marker.apart_lon}}
            ...
            if ( this.state.markerNowFocus === 'article' &&  this.props.mainMapViewTarget.agent ) {
                const iosAgentCaption = {
                    textSize: 14,
                    color: '#ffffff',
                    haloColor : '#000000',
...
const styles = StyleSheet.create({
    ...
    showButtonWrap : {
        position:'absolute',left:10,bottom:10, height:CommonUtil.dpToSize(45*2), width:80,backgroundColor:'transparent',zIndex:99
    },
    showButton : {

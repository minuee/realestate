...
class IntroScreen extends React.Component {
    ...
    setUpdatePermision2 = () => {
        setTimeout(() => {this.setState({moreLoading:true,popLayerView:false,})},100)
        setTimeout(() => {this.setState({moreLoading:false})},120)
    ...
    closePopLayer = async( mode = null) => {
        ...
        if ( mode ) {
            ...
            console.log('this.state.markerDataAgent.length',this.state.markerDataAgent.length)
            if ( this.state.markerDataAgent.length == 0 ) {
                ...
                const markerDataAgentTmp = this.state.markerDataAgent;
                const markerDataArticleTmp = this.state.markerDataArticleTmp;
                if ( this.props.mainMapViewTarget.maemul  ) {
                    const returnArray = await markerDataArticleTmp.concat(markerDataAgentTmp);                        
                    this.setState({
                        markerDataArticle : returnArray
                    })
                }else{
                    this.setState({
                        markerDataArticle : markerDataAgentTmp
                    })
                }
    ...
    reloadArticle = async() => {
        ...
        if ( this.props.mainMapViewTarget.agent) {
            const markerDataAgentTmp = this.state.markerDataAgent;                   
            const returnArray = await copyyfilterData.concat(markerDataAgentTmp);             
            this.setState({
                markerDataArticle : returnArray,
                markerDataArticleTmp : copyyfilterData
            })
        }else{
            this.setState({
    ...
    componentDidMount() {       
        const ddd = "에스뷔부동산3".match(/.{1,3}/g);
        console.log('dddd',ddd)
        this.props._toggleMainMapOpenTarget({agent:false,maemul:true})
    }
    ...
    toggleData = async( mode, bool) => {
        if ( mode === 'M') {     
            console.log('boolboolbool.maemul',bool)      
            const markerDataArticleTmp = this.state.markerDataArticleTmp;
            const markerDataAgentTmp = this.state.markerDataAgent;
            if ( bool === false  ) {//
                if ( this.props.mainMapViewTarget.agent ) {
                    const returnArray = await markerDataArticleTmp.concat(markerDataAgentTmp);     
                    await this.setState({ moreLoading:true,markerDataArticle : returnArray})
                }else{
                    await this.setState({ moreLoading:true,markerDataArticle : markerDataArticleTmp})
                }
            }else{//감추기
                if ( this.props.mainMapViewTarget.agent ) {
                    await this.setState({ moreLoading:true,markerDataArticle : markerDataAgentTmp})
                }else{
                    await this.setState({ moreLoading:true,markerDataArticle : []})
                }
            ...
            if ( bool === false  ) {
                ...
                const markerDataArticleTmp = this.state.markerDataArticleTmp;
                //const returnArray = await markerDataArticleTmp.filter((info)=> info.sub_gubun != 'agent');  
    ...
    render() {
        if ( this.state.loading ) {
            ...
            if ( this.props.mainMapViewTarget.maemul || this.props.mainMapViewTarget.agent) {  
                if ( this.state.markerNowFocus === 'sigungu') {
                    ...
                    markerData = this.state.markerDataSido;
                }
            }else{
                markerData = [];
            }
                ...
                <View style={ styles.container }>                    
                    ...
                    <View style={{flex:1}}> 
                        <MapView
                            ...
                            pitchEnabled={!this.state.moreLoading} rotateEnabled={!this.state.moreLoading} zoomEnabled={!this.state.moreLoading} scrollEnabled={!this.state.moreLoading}
                            initialRegion={{
                        ...
                        >
                            ...
                            markerData.map((marker,index) => (      
                                ...
                                <Marker
                                    ...
                                    style={{width: marker.sub_gubun === 'agent' ? CommonUtil.dpToSize(81) : marker.gubun === 'article' ? 110 : 86*1.14}}
                        ...
                        */}
                        <View style={styles.showButtonWrap}>
                            ...
                            {
                                this.state.markerNowFocus !== '1article' ?

...
export default class AppIntroSlider extends React.Component {
  static defaultProps = {
    activeDotStyle: {
      backgroundColor: DEFAULT_COLOR.base_color//'rgba(255, 255, 255, .9)',
    },
    dotStyle: {
      backgroundColor: '#111'//'rgba(0, 0, 0, .2)',
    },
    skipLabel: '건너뛰기',
    doneLabel: '시작하기',
    nextLabel: '다음',
  ...
  _renderDefaultButton = name => {
    ...
    if (this.props.bottomButton) {
      content = (
        <View
          style={[
            styles.bottomButton,this.props.buttonStyle,
            (name === 'Skip' || name === 'Prev') && {
  ...
  _renderPagination = () => {
    ...
    const skipBtn = (!isFirstSlide && this._renderPrevButton()) || (!isLastSlide);

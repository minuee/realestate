...
class IntroduceApp extends React.Component {
    ...
    _renderItem = ({ item }) => {
        if ( item.key === 's1') {
            ...
            return (
                <View style={{backgroundColor: item.backgroundColor,width:SCREEN_WIDTH,height:SCREEN_HEIGHT}}>
                     <View style={{position:'absolute',right:10,top:10,height:70,width:100,justifyContent:'flex-end',zIndex:100}}>
                        <TouchableOpacity 
                            onPress={() => this.nonUserLogin('A')}
                            style={styles.buttonWrapOn2}
                        >
                            <CustomTextM style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize14),color:'#fff'}}>둘러보기</CustomTextM>
                        </TouchableOpacity>
                    </View>
                    <View style={{flex:1,paddingHorizontal:50,justifyContent:'center',alignItems:'center'}}>
                        <Image
                    ...
                    <View style={{flex:1,paddingTop:40,paddingHorizontal:30,justifyContent:'flex-start',alignItems:'center'}}>                        
                        <CustomTextM style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize25),color:'#363636'}}>{item.mainTitle}</CustomTextM>  
                        <CustomTextM style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize25),color:DEFAULT_COLOR.base_color}}></CustomTextM>                       
                        <CustomTextL style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize20),color:'#363636'}}>{item.subTitle}</CustomTextL>   
                        <TouchableOpacity 
                            onPress={() => this.nonUserLogin('B')}
                            style={styles.buttonWrapOn}
                        >
                            <CustomTextM style={{fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.fontSize14),color:'#fff'}}>회원가입/로그인</CustomTextM>
                        </TouchableOpacity>                    
                    </View>

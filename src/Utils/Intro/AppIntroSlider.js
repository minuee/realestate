...
const isIphoneX = CommonFunction.isIphoneX();
...
export default class AppIntroSlider extends React.Component {
  static defaultProps = {
    activeDotStyle: {
      backgroundColor: DEFAULT_COLOR.lecture_base//'rgba(255, 255, 255, .9)',
    },
    dotStyle: {
      backgroundColor: DEFAULT_COLOR.base_color_666//'rgba(0, 0, 0, .2)',
    },
    skipLabel: '건너뛰기',
    doneLabel: '시작하기',
    nextLabel: '다음',
  ...
  _renderPagination = () => {
    ...
    if ( totalSlide === parseInt(this.state.activeIndex)+parseInt(1) ) {
      return (
        <View style={[styles.paginationContainer2,{backgroundColor:DEFAULT_COLOR.lecture_base,alignItems:'center',justifyContent:'center'}]}>
            <TouchableOpacity onPress={() => this.props.onSkip()}>
              <Text style={{color:DEFAULT_COLOR.base_color_fff,fontSize: PixelRatio.roundToNearestPixel(DEFAULT_TEXT.head_medium)}}>시작하기</Text>
            </TouchableOpacity>
      ...
      return (
        <View style={[styles.paginationContainer, this.props.paginationStyle]}>          
          <View style={{position:'absolute',left:0,top:0,height:3,width:parseInt(nowSlide/totalSlide*100) + '%',backgroundColor:DEFAULT_COLOR.lecture_base}} />
...
const styles = StyleSheet.create({
  ...
  buttonText: {
    backgroundColor: 'transparent',
    color: DEFAULT_COLOR.base_color_666,//'white',
